/*Assignment26 26Nov Please cover below annotations
 *  @Test (alphabetical order)
 *  Priority(priority=1)
 *  dependsOnMethods : @Test(dependsOnMethods="m1")
 *  alwaysRun : @Test(alwaysRun = true)
 *  assign Groups to the test, run them from 
 *  testng.xml enabled : @Test(enabled=false)
...
public class Assignment26 {
	...
	void m1() {
		System.out.println(
				"As per alphabetical order, it should have been executed before test1. But as we added dependsOnMethod = test1, test1 will execute before m1");
	}

	@Test(priority = 1, groups = "sanity")
	void test4() {
		System.out.println("Higher the number lower the priority. It will execute at the end in sequence");
	}
